...
RANDOM_STATE = 42
MAX_ROWS_FOR_STACKING = 4_000_000
SAFE_THREADS = "1"
os.environ.update({
...
df['AddrProb'] = df['SrcAddr'].map(addr_prob_map).fillna(df['DstAddr'].map(addr_prob_map)).fillna(0.0)

# ------------------------------
# Per-sensor C&C detection + 3D plotting based on degree-graph (paper)
# ------------------------------
detected_summary = []
for sid in sorted(df["SensorId"].unique()):
    print(f"\n=== [Sensor {sid}] Auto C&C Detection (graph-based, IsolationForest) ===")
    ...
    # If there are too few nodes, fallback to simple percentile rule
    if len(stats) < 6:
        # fallback thresholds (conservative)
        prob_thr = max(0.15, np.percentile(stats["avg_prob"].values, 60) if len(stats) > 0 else 0.15)
        deg_thr = max(3, np.percentile(stats["degree"].values, 75) if len(stats) > 0 else 3)
        ratio_thr = max(0.5, np.percentile(stats["out_ratio"].values, 60) if len(stats) > 0 else 0.5)
        print(f"[Fallback Threshold][Sensor {sid}] prob>={prob_thr:.3f} | degree>={deg_thr:.1f} | out_ratio>={ratio_thr:.2f}")

        node_roles = {}
        for n, r in stats.iterrows():
            if (r["avg_prob"] >= prob_thr) and (r["degree"] >= deg_thr) and (r["out_ratio"] >= ratio_thr):
                node_roles[n] = "C&C"
            else:
                node_roles[n] = "Normal"
    else:
        # Build feature matrix for IsolationForest
        feat_if = stats[["avg_prob", "degree", "out_ratio"]].copy().fillna(0).values

        # Scale features for IF
        if_scaler = StandardScaler()
        feat_if_scaled = if_scaler.fit_transform(feat_if)

        # adaptive contamination: try to estimate plausible fraction of anomalies
        # rule: cont = min(0.15, max(0.01, 5/len(stats)))
        cont = min(0.15, max(0.01, 5.0 / max(1, len(stats))))
        iso = IsolationForest(n_estimators=256, contamination=cont, random_state=RANDOM_STATE, behaviour="new")
        iso.fit(feat_if_scaled)
        preds = iso.predict(feat_if_scaled)  # -1 anomaly, 1 normal
        scores = -iso.score_samples(feat_if_scaled)  # higher => more anomalous

        # Build adaptive gating thresholds to avoid extremely low-degree false positives
        # degree_min = max(3, 25th percentile of degrees)
        degree_min = max(3, int(np.percentile(stats["degree"].values, 25)))
        # prob_min = max(0.05, median(avg_prob) * 0.5)
        prob_median = np.median(stats["avg_prob"].values)
        prob_min = max(0.05, prob_median * 0.5)

        # collect node roles from IF predictions + gating
        node_roles = {}
        anomalies = 0
        for (n, row), pred, score in zip(stats.iterrows(), preds, scores):
            r = row
            is_anom = (pred == -1)
            if is_anom and (r["degree"] >= degree_min) and (r["avg_prob"] >= prob_min):
                node_roles[n] = "C&C"
                anomalies += 1
            else:
                node_roles[n] = "Normal"

        print(f"[IF][Sensor {sid}] contamination={cont:.3f} | anomalies_detected={anomalies} | degree_min={degree_min} | prob_min={prob_min:.3f}")
...
print("\nDone. Graph-based pipeline + stacking model + per-sensor 3D degree-graph exports complete.")
log_ram("Script End")
