...
for sid in sorted(df["SensorId"].unique()):
    ...
    if cc_nodes:
        ...
        # Bar: Top C&C candidates by cnc_score
        bar_fig = px.bar(
            cnc_top,
            x="ip",
            y="cnc_score",
            hover_data=["avg_prob", "degree", "in_ratio", "out_ratio"],
            title=f"Sensor {sid} – Top C&C Candidates (CNC Score)",
            labels={"ip": "Node (IP)", "cnc_score": "CNC Score"}
        )
        bar_fig.update_layout(xaxis_tickangle=-45)
        bar_path = os.path.join(output_dir, f"Sensor{sid}_CNC_Score_{fileTimeStamp}.html")
        bar_fig.write_html(bar_path)

        # Scatter: Avg_prob vs Degree (bubble size = cnc_score)
        scatter_fig = px.scatter(
            ...
            title=f"Sensor {sid} – AvgProb vs Degree (C&C Candidates)",
            labels={"degree": "Degree (#flows)", "avg_prob": "Avg Malicious Probability"},
        )
        scatter_path = os.path.join(output_dir, f"Sensor{sid}_CNC_Scatter_{fileTimeStamp}.html")
        scatter_fig.write_html(scatter_path)

        print(f"[Plot] Sensor {sid} plots saved ->\n{bar_path}\n{scatter_path}")
