...
for sid in sorted(df["SensorId"].unique()):
    print(f"\n=== [Sensor {sid}] Auto C&C Detection (graph-based) ===")
    ...
    # AUTO THRESHOLD (adaptive per sensor)
    probs = stats["avg_prob"].values
    degrees = stats["degree"].values
    ratios = stats["out_ratio"].values

    # ---- Probability threshold (Hybrid Otsu + Percentile) ----
    prob_thr_otsu = otsu_threshold(probs)
    prob_thr_pct  = np.percentile(probs, 70) if len(probs) > 10 else 0.2
    prob_threshold = max(prob_thr_otsu, prob_thr_pct, 0.15)

    # ---- Degree threshold ----
    degree_threshold = np.percentile(degrees, 80) if len(degrees) > 5 else 5
    degree_threshold = max(degree_threshold, 5)

    # ---- Out-ratio threshold ----
    ratio_threshold = np.percentile(ratios, 70) if len(ratios) > 5 else 0.5
    ratio_threshold = max(ratio_threshold, 0.50)

    print(f"[Auto Threshold][Sensor {sid}] prob>={prob_threshold:.3f} | degree>={degree_threshold:.1f} | out_ratio>={ratio_threshold:.2f}")
