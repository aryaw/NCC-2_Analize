...
for sid in sorted(df["SensorId"].unique()):
    ...
    if cc_nodes:
        ...
        print("\n[C&C Summary Table]")
        cnc_table = cnc_df[["avg_prob", "degree", "in_ratio"]].reset_index()
        cnc_table = cnc_table.rename(columns={"index": "C&C_IP"})
        print(cnc_table.to_string(index=False))

        print("[C&C IPs]")
        print(cnc_df.index.tolist())
        ...
        cc_ip_list = ", ".join(cnc_df.index.astype(str).tolist()[:5])
        if len(cnc_df) > 5:
            cc_ip_list += ", ..."
        ...
        fig = go.Figure(
            ...
            layout=go.Layout(
                ...
                title=(
                    f"Sensor {sid} – 3D Network Graph (Safe, {len(G.nodes())} nodes)"
                    f"<br><sup>C&C IPs: {cc_ip_list}</sup>"
                ),
                showlegend=False,
