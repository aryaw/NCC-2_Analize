...
train_feat_path = os.path.join(outputdata_dir, f"train_features_{fileDataTimeStamp}.csv")
train_label_path = os.path.join(outputdata_dir, f"train_labels_{fileDataTimeStamp}.csv")
test_feat_path  = os.path.join(outputdata_dir, f"test_features_{fileDataTimeStamp}.csv")
test_label_path = os.path.join(outputdata_dir, f"test_labels_{fileDataTimeStamp}.csv")

pd.DataFrame(X_train).to_csv(train_feat_path, index=False)
pd.DataFrame(y_train, columns=["Label"]).to_csv(train_label_path, index=False)
pd.DataFrame(X_test).to_csv(test_feat_path, index=False)
pd.DataFrame(y_test, columns=["Label"]).to_csv(test_label_path, index=False)
